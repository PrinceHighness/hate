	...
	<script type="text/javascript">
		...
		function init_user_dg() {
			...
			$('#user_dg').bootstrapTable({
				url:"<%=request.getContextPath() %>/user/selectUserListJson.jhtml",
				dataType:"json",
				//请求方式
				method:"post",
				//必须的，不然会造成中文乱码
		...
		function edit_user_role(user_id) {
			BootstrapDialog.show({
				title:"用户管理>>用户赋角色",
				message: $('<div></div>').load('<%=request.getContextPath() %>/user/toUserRolePage.jhtml?userID=' + user_id),
				buttons: [{
	                label: '确定',
	                ...
	                action: function(dialogItself){
	                	...
	                	//使用ajax保存结果
	                	$.ajax({
	                		url:"<%=request.getContextPath() %>/user/insertUserRoleList.jhtml",
